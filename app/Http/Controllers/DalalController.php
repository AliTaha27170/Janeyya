...
class DalalController extends Controller
{
    public function add()
    {
        return view('main.Store&Edit.Dalal');
    }
 
//*START* Store new Dalal   >>>>>>>>>   >>>>>>>>>   >>>>>>>>>
...
//*END* Store new Dalal   >>>>>>>>>   >>>>>>>>>   >>>>>>>>>
    ...
    {
        $user=User::where('id',$id)->where('company',auth()->user()->id)->where('role',5)->first();
        if(!$user)
            return back();
       
        return view('main.Store&Edit.Dalal')->with('user',$user);
    }
    ...
    public function update(Request $request,$id)
    {
        ...
        if($user->company != auth()->user()->id && $user->role ==5 )
            return back();
